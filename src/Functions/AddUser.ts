...
export const getUserByEmail = async (userEmail: string | undefined) => {
    const response = await fetch(
      `/api/Employee/GetEmployeeByEmail/${userEmail}`
    );
...
export const createNewUser = async (newUser: EmployeeDTO) => {
    try {
      const response = await httpRequest("/api/Employee/PostEmployee", newUser, "POST");
